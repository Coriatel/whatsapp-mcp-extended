...
func TestClassifyReceipt(t *testing.T) {
	...
	cases := []struct {
		...
		{"self-chat read-self on an AD JID forwarded as read",
			receiptOn(types.JID{User: ownPN.User, Server: types.DefaultUserServer, Device: 23}, types.ReceiptTypeReadSelf), "read", true, true},
		{"self-chat read-self on the hosted server forwarded as read",
			receiptOn(types.NewJID(ownPN.User, types.HostedServer), types.ReceiptTypeReadSelf), "read", true, true},
		{"self-chat read-self on hosted.lid forwarded as read",
			receiptOn(types.NewJID(ownLID.User, types.HostedLIDServer), types.ReceiptTypeReadSelf), "read", true, true},
		{"other-chat read-self dropped", receiptOn(other, types.ReceiptTypeReadSelf), "", false, false},
		{"other user on the hosted server dropped",
			receiptOn(types.NewJID(other.User, types.HostedServer), types.ReceiptTypeReadSelf), "", false, false},
		{"own PN digits on the LID server dropped",
			receiptOn(types.NewJID(ownPN.User, types.HiddenUserServer), types.ReceiptTypeReadSelf), "", false, false},
		{"group read-self dropped",
			receiptOn(types.NewJID("120363000000000000", types.GroupServer), types.ReceiptTypeReadSelf), "", false, false},
